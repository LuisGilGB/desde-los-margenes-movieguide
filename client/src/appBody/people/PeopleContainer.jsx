...
const PeopleViewport = (props) => {
  ...
  return (
    <Container layout="colflex">
      ...
      >
        <LinkButton to={ROUTES.PEOPLE.NEW_PERSON_FORM}>Add person</LinkButton>
      </Container>
      <Container flex={1} layout="fit">
        <Switch>
          <Route
            path={ROUTES.PEOPLE.NEW_PERSON_FORM}
            exact
            render={() => <NewPersonForm data={{}} />}
          />
          <Route
            path={ROUTES.PEOPLE.DETAIL}
            exact
...
const mapDispatchToProps = (dispatch, props) => ({
  ...
  loadPersonData: (personId) =>
    dispatch(actionCreators.loadPersonData(personId)),
  goToPersonDetail: (personSelected) =>
    dispatch(
      navLogicActionCreators.navigateWithPush(
        ROUTES.PEOPLE.DETAIL,
        personSelected,
      ),
    ),
});
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(PeopleViewport),
);
