  ...
  return new Promise((resolve, reject) => {
    if (isValid) {
      Country.findOneAndUpdate(
        ...
        .then((foundCountry) => resolve(foundCountry))
...
router.post('/', (req, res) => {
  ...
  if (Array.isArray(body)) {
    const badCountry = body.find((country) => {
      const { isValid, errors: validatorErrors } = validateCountryInput(
        country,
      );
      if (!isValid) {
        errors = {
    ...
    const { isValid, errors: countryValidationErrors } = validateCountryInput(body);
    if (!isValid) {
      return res.status(400).json(countryValidationErrors);
    }
