...
const { getPersonSchemaAttributes } = require('../../models/person/functions');

// Import validators
const validatePersonToAdd = require('../../validation/person/addPerson.js');

const test = () => (
  new Promise((resolve) => resolve({ msg: 'People path works' }))
);
...
const checkAliasSlugExists = (slug) => (
  new Promise((resolve, reject) => {
...
const getAvailableAlias = (slug, tryNum = 0) => (
  new Promise((resolve, reject) => {
    if (tryNum > 15) {
      return reject(
        'More than 16 unsuccessful attempts of getting a unique alias? Something may not be working right (we put this limit as a protection against infinite loops).',
      );
    }
    ...
    checkAliasSlugExists(slugDraft)
      .then((match) => (
        match
          ? resolve(getAvailableAlias(slug, tryNum + 1))
          : resolve(slugDraft)
      ))
      .catch((err) => reject(err));
  })
    ...
    Person.findOneAndUpdate(
      ...
      .catch((err) => {
        ...
        reject({
          ...
          errors: {
            msg:
              'There was an error in the database, please contact the web administrator',
          },
        });
      });
  })
